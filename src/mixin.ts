...
@Component
export class XStateMixin<TContext, TStateSchema extends StateSchema, TEvents extends EventObject> extends Vue {
  @Prop()
  ...
  private $xStateStateMachine: StateNode<TContext, TStateSchema, TEvents> | null = null;

  private $xStateInterpreter: Interpreter<TContext, TStateSchema, TEvents> | null = null;
  ...
  public xStateInit(
    ...
    this.$xStateStateMachine = Machine<TContext, TStateSchema, TEvents>(machineConfig, {}, initialContext).withConfig(
      configOptions,
    );
    this.$xStateInterpreter = interpret<TContext, TStateSchema, TEvents>(this.$xStateStateMachine);
    this.$xStateSubscription = this.channel
      ? this.channel.subscribe(action => this.$xStateInterpreter?.send(action))
      : null;
  ...
  public dispatch(action: TEvents): void {
    if (this.$xStateInterpreter) {
      this.$xStateInterpreter.send(action);
    } else {
      console.warn(
        `%cx-state mixin: %cThe machine was not initialized, please use %cinitState() %con %ccreated() %cfor it`,
  ...
  private $xStateInitInterpreter(): void {
    this.$xStateInterpreter?.start();

    this.$xStateInterpreter?.onTransition(newState => {
    ...
    this.$xStateInterpreter?.onChange(context => {
